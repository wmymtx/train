...
using RIA.Train.Migrations.SeedData;
...
namespace RIA.Train.Tests
{
    public abstract class TrainTestBase : AbpIntegratedTestBase<TrainTestModule>
    {
        ...
        protected void UsingDbContext(Action<TrainDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TrainDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TrainDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TrainDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TrainDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TrainDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TrainDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TrainDbContext, Task<T>> func)
        {
            T result;
